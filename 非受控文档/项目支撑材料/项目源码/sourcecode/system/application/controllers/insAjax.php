 ...
 */
	function getInsInfor(){
		...
		$insId = $_POST['insId'];
		$ins = $this->instancemodel->getInstanceByid($insId);
		if(!empty($ins)){
			$tmp = $this->instancemodel->getRolesInfo($insId);
			$playerNum = 0;
 ...
 */
	function changeRoleStatus(){
		...
		if($this->validateRole($insId,$roleId)){ //For extend
			if($changeFlag == CLOSE_ROLE && $this->instancemodel->closeRole($insId,$roleId)){
				$output = $this->xmlmodel->xmlFactoryElement('0',"关闭角色成功");
				$output = $this->xmlmodel->xmlFactoryEndElement($output);
				$flag = TRUE;
			}
			else if($changeFlag == OPEN_ROLE && $this->instancemodel->openRole($insId,$roleId)){
 ...
 */	
	function changeRolePlayer(){
		...
		if($this->validateRole($insId,$roleId) && $this->validateUser($userId)){ //For extend
			$this->instancemodel->acceptRole($insId,$roleId,$userId);
			...
			$applyingList = $this->instancemodel->getApplyer($insId,0,$roleId);
 ...
 */
	function applyOp(){
		...
		switch($flag){
			...
			if(!$this->instancemodel->isInProject($insId,$userId)
			&& ($this->instancemodel->getApplyingRoleid($insId,$userId) == '-1')
				&& $this->instancemodel->applyRole($insId,$roleId,$userId)){
			...
			case OP_CANCEL:	//Apply to cancel a role apply
				if($this->instancemodel->isAppliedRoleid($insId,$roleId,$userId)
				&& $this->instancemodel->quitApply($insId,$userId)){
			...
			if($userId == $this->instancemodel->getRolePlayerid($insId,$roleId)
 ...
 */
	function banUser(){
		...
		$insId = $_POST['insId'];
		$roleId = $_POST['roleId'];
		$userId = $this->instancemodel->getRolePlayerid($insId,$roleId);
		//需要对用户权限做一个判断
		//if()	
		if($this->instancemodel->banUser($insId,$userId)){
 ...
 */
	function changeMyRole(){
		...
		//需要对用户权限进行判断
		//if(user)
		if($this->instancemodel->banUser($insId,$this->session->userdata('user_id')) &&
			$this->instancemodel->acceptRole($insId,$roleId,$this->session->userdata('user_id'))
		){
 ...
 */
	function applyIndicator(){
		...
		$insId = $_POST['insId'];
		$userId = $_POST['userId'];
		if($this->instancemodel->isApplyingIndicator($insId,$userId) || $this->instancemodel->isIndicator($insId,$userId)){
			echo $this->returnXmlFactory(ERROR,"您正在申请指导者角色或者您已经担任项目指导者角色");
			exit;
		}else{
			if($this->instancemodel->applyIndicator($insId,$userId)){
 ...
 */
	function denyApplyIndicator(){
		...
		$insInfo = $this->instancemodel->getInstanceByid($insId);
		if($insInfo['creatorid'] != $myUserId ){
			echo $this->returnXmlFactory(ERROR,"无法完成操作：您不是该项目创建者");
			exit;
		}
		if(!$this->instancemodel->isApplyingIndicator($insId,$userId)){
			echo $this->returnXmlFactory(ERROR,"无法完成操作：该用户未申请担任项目指导者");
			exit;
		}
		if($this->instancemodel->denyApplyIndicator($insId,$userId)){
 ...
 */
	function acApplyIndicator(){
		...
		if($this->instancemodel->acApplyIndicator($insId,$userId)){
			$applyers = $this->instancemodel->getIndicatorApplyers($insId);
			$output = $this->xmlmodel->xmlFactoryElement(SUCCESS,"操作成功");
			$user = array(
				'userName' => $this->usermodel->getUsername($userId),
				'userId' => $userId
 ...
 */
	function cancelApplyIndicator(){
		...
		if($this->instancemodel->cancelApplyIndicator($insId,$userId)){
			echo $this->returnXmlFactory(SUCCESS,"操作成功！");
			exit;
		}else{
 ...
 */
	function cancelIndicatorAct(){
		...
		&&$this->instancemodel->cancelIndicatorAct($insId,$userId)){
