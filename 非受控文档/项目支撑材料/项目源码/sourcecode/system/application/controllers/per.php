...
*/
	...
	if($this->isLoggedIn() == FALSE){
		...
		while($value = current($tmp)){
			$apply_per = $this->instancemodel->getApplying($value['instanceid']);
			$playing = $this->instancemodel->getPlaying($value['instanceid']);
			...
			$is_started = $this->instancemodel->isstarted($value['instanceid']);
