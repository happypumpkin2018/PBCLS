...
// ------------------------------------------------------------------------
	...
	function getActorid($instanceid)
	{
...
// ------------------------------------------------------------------------
	 ...
	 * @return	array
	 */
	function getNewSubmits($instanceid)
	{
		$this->db->where('instanceid', $instanceid);
		$this->db->where('ischecked', 0);
		return $this->db->get('newsubmits')->result_array();
...
// ------------------------------------------------------------------------
	 ...
	 * @return	array
	 */
	function getAcceptedSubmits($instanceid)
	{
//		$this->db->select('file','submittime','uploader','accepttime');
		$this->db->where('instanceid', $instanceid);
	...
	function getDeniedSubmits($instanceid)
	{
		$this->db->where('instanceid', $instanceid);
 ...
 */
	...
	function isInProject($ins_id,$user_id){
		$user_id = $this->session->userdata('user_id');
		if($this->isIndicator($ins_id,$user_id)){
			return TRUE;
		}
		$actors = $this->getActorid($ins_id);
...
/**
 * 功能：判断一个用户在某个项目中是否为指导者（indicator)
 * @param $ins_id: 项目ID
 * @param $user_id： 用户ID
 * @return TRUE or FALSE
 */	
	
	function isIndicator($insId,$userId){
		$sql = "SELECT * from instance_indicator WHERE instanceid=$insId AND userid=$userId AND status='1'";
		$query = $this->db->query($sql);
...
/*		$this->db->where('instanceid', $insId);
		...
		$result = $this->db->get('instance_indicator')->row_array();
